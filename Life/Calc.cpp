...
void Run(bool &RunCalc, bool &CalcEnd, std::unordered_map <LONGLONG, POINT> &LifePoint)
{
	int n[3] = { -1, 0, 1 };
	POINT tmpPoint;// = { 0,0 };
	Calc calc;
...
void Calc::RunLife(bool &RunCalc, bool &CalcEnd)

{
	//InitializeCriticalSection(&cs);
	std::vector<std::thread> thr(2); //для запуска двух потоков
	for (int i = 1; i <= 1; i++) thr[i - 1] = std::thread(Run, std::ref(RunCalc), std::ref(CalcEnd), std::ref(this->LifePoint));
